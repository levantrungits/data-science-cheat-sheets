...
key_entry = ctk.CTkLabel(input_frame, text="OPENAI_API_KEY")
key_entry.grid(row=0, column=0, padx=10, pady=10)
key_entry_entry = ctk.CTkTextbox(input_frame, height=10)
key_entry_entry.grid(row=0, column=1, padx=10, pady=10)
...
style_label.grid(row=2, column=0, padx=10, pady=10)
style_dropdown = ctk.CTkComboBox(input_frame, values=["Realistic", "Cartoon", "3D Illustration", "Flat Art"])
style_dropdown.grid(row=2, column=1, padx=10, pady=10)
...
number_slider.grid(row=3, column=1)

generate_button = ctk.CTkButton(input_frame, text="Generate", command=generate)
generate_button.grid(row=4, column=0, padx=10, pady=10, sticky="news", columnspan=2)
